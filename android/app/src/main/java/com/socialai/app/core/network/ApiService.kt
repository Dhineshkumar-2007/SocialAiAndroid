...
@Serializable data class CheckDuplicateRequest(val title: String, val description: String)
@Serializable data class DuplicateCheckResponse(val isDuplicate: Boolean, val similarity: Double = 0.0, val matchedProblemId: String? = null, val matchedTitle: String? = null)
...
@Serializable data class VerifyRequest(val isResolved: Boolean, val rating: Int, val notes: String? = null)
interface ApiService {
    ...
    @POST("problems") suspend fun createProblem(@Body request: CreateProblemRequest): CreateProblemResponse
    @POST("problems/check-duplicate") suspend fun checkDuplicate(@Body request: CheckDuplicateRequest): DuplicateCheckResponse
